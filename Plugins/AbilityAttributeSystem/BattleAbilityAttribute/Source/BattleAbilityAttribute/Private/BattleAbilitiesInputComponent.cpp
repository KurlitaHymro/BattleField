...
void UBattleAbilitiesInputComponent::SetupPlayerControls_Implementation(UEnhancedInputComponent* PlayerInputComponent)
{
	...
	for (auto& Binding : MappedAbilities)
	{
		...
		// Pressed event
		InputComponent->BindAction(InputAction, ETriggerEvent::Started, this, &UBattleAbilitiesInputComponent::OnAbilityInputPressed, InputAction);

		// Released event
		InputComponent->BindAction(InputAction, ETriggerEvent::Completed, this, &UBattleAbilitiesInputComponent::OnAbilityInputReleased, InputAction);
	}
}
...
void UBattleAbilitiesInputComponent::SetupBindings()
{
...
void UBattleAbilitiesInputComponent::TeardownBindings()
{
	// 移除增强输入操作代理
	if (InputComponent)
	{
...
void UBattleAbilitiesInputComponent::OnAbilityInputPressed(UInputAction* InputAction)
{
...
void UBattleAbilitiesInputComponent::OnAbilityInputReleased(UInputAction* InputAction)
{
	// The AbilitySystemComponent may not have been valid when we first bound input... try again.
	if (!AbilityComponent)
	{
...
void UBattleAbilitiesInputComponent::SetupBinding(UInputAction* InputAction, FGameplayAbilitySpecHandle AbilityHandle)
{
	if (!AbilityComponent)
	{
		return;
	...
	if (AbilitySpec)
	{
		...
		if (InputComponent)
		{
			...
			{
				Binding->OnPressedHandle = InputComponent->BindAction(InputAction, ETriggerEvent::Started, this, &UBattleAbilitiesInputComponent::OnAbilityInputPressed, InputAction).GetHandle();
			}

			if (Binding->OnReleasedHandle == 0)
			{
				Binding->OnReleasedHandle = InputComponent->BindAction(InputAction, ETriggerEvent::Completed, this, &UBattleAbilitiesInputComponent::OnAbilityInputReleased, InputAction).GetHandle();
			}	
		}
	}
}

void UBattleAbilitiesInputComponent::TeardownActionBinding(UInputAction* InputAction)
{
	if (!AbilityComponent)
	{
		return;
...
void UBattleAbilitiesInputComponent::TeardownAbilityBinding(FGameplayAbilitySpecHandle AbilityHandle)
{
	if (!AbilityComponent)
	{
		return;
